...
class LispUser(lisp: Lisp) {
  ...
  def run = {
    ...
    Console.println;
    // read the lisp test from a file
    import scala.io._
    import sys.process._

    val pwd= System.getProperty("user.dir")
    // convert the file in to string 
    val lispFileContent: String = Source.fromFile(s"${pwd}/src/main/scala/lisp-test.lisp").getLines.mkString
    Console.println("lisp-test = " + evaluate( lispFileContent))
  }
}
...
// val expectedLisp = """
// (def parse (pa-list)
// (map #'identity par-list))
//(parse (quote (1 2 3 4 5)))"""


//new LispUser(LispCaseClasses).evaluate(s"${expectedLisp}")
